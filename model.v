...
Section MachineState.
  ...
  Definition start_mode := match X with | [] => ConflictFree | _ => Replay end.
...
Section Machine.
  ...
  Definition get_state_history (s : state) :=
    s.(oracle_performed) ++ combined_histories s.(Y_performed) ++ s.(X_performed).
  Definition state_with_md (s : state) (md : mode) :=
    mkState s.(X_copy) s.(Y_copy) s.(X_performed) s.(Y_performed) s.(oracle_performed) md.
  ...
  Definition get_oracle_response (s : state) (t: tid) (i: invocation) : state * action :=
    ...
    (mkState (s.(X_copy)) (s.(Y_copy)) (s.(X_performed)) (s.(Y_performed)) ((t,i,resp)::s.(oracle_performed)) Oracle,
     (t,i,resp)).
  ...
  Definition get_commute_response (s : state) (t: tid) (i: invocation) : state * action :=
    match rev (s.(Y_copy) t) with
      | hd::tl => (mkState s.(X_copy)
                               ...
                               s.(X_performed) (fun tid => if tid =? t then hd::(s.(Y_performed) t)
                                                    else s.(Y_performed) tid)
                                        s.(oracle_performed) ConflictFree, hd)
  ...
  Definition get_replay_response (s : state) (t: tid) (i : invocation) : state * action :=
    ...
    | hd::tl => (mkState (rev tl) s.(Y_copy) (hd::s.(X_performed)) s.(Y_performed) s.(oracle_performed) s.(md), hd)
    | _ => (mkState s.(X_copy) s.(Y_copy) ((t,i,NoResp)::s.(X_performed)) s.(Y_performed) s.(oracle_performed) s.(md),
           (t,i,NoResp)) (* should never hit this *)
    end.
  Definition next_mode (s : state) (t: tid) (i: invocation) : mode :=
    match s.(md) with
      | Oracle => Oracle
      | ConflictFree => match rev (s.(Y_copy) t) with
                     | [] => Oracle
                     | hd :: tl => if action_invocation_eq hd t i then ConflictFree
                                   else Oracle
                   end
  ...
  Definition machine_act (s : state) (t: tid) (i : invocation) : (state * action) :=
    ...
    match mode with
      ...
      | ConflictFree => get_commute_response (state_with_md s ConflictFree) t i
      | Replay => match rev (s.(X_copy)) with
                  | [hd] => get_replay_response (state_with_md s ConflictFree) t i
                                                (* Changes mode to ConflictFree *)
